...
bool FScriptCodeGeneratorBase::SaveHeaderIfChanged(const FString& HeaderPath, const FString& NewHeaderContents)
{
	...
	if (bHasChanged)
	{
		...
		{
			UE_LOG(LogUnrealDotNetGenerator, Warning, TEXT("Failed to save header export: '%s'"), *TmpHeaderFilename);
		}
		else
		{
...
void FScriptCodeGeneratorBase::RenameTempFiles()
{
	...
	for (auto& TempFilename : TempHeaders)
	{
		...
		{
			UE_LOG(LogUnrealDotNetGenerator, Error, TEXT("%s"), *FString::Printf(TEXT("Couldn't write file '%s'"), *Filename));
		}
		else
		{
			UE_LOG(LogUnrealDotNetGenerator, Log, TEXT("Exported updated script header: %s"), *Filename);
		}
	}
}
...
bool FScriptCodeGeneratorBase::CanExportFunction(const FString& ClassNameCPP, UClass* Class, UFunction* Function)
{
	...
	if (!Function->HasAllFunctionFlags(FUNC_Public))
	{
		return false;
	}
